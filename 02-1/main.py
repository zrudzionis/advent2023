...
for i, line in enumerate(lines):
  ...
  for game in games:
    ...
    gameResult = { 'r': 0, 'g': 0, 'b': 0 }
    for gamePart in gameParts:
      cnt, color = gamePart.split(' ')
      cnt = int(cnt)
      gameResult[color[0]] += cnt
    gameMax = gamesMax.get(gameId, { 'r': 0, 'g': 0, 'b': 0 })
    for color in ['r', 'g', 'b']:
      gameMax[color] = max(gameResult[color], gameMax[color])
    gamesMax[gameId] = gameMax

mx = { 'r': 12, 'g': 13, 'b': 14 }
s = 0

for gameId, gameMax in gamesMax.items():
  add = True
  for color in ['r', 'g', 'b']:
    if gameMax[color] > mx[color]:
      add = False
  if add:
    s += gameId
